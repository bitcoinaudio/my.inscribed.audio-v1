...
export default function ConnectWallet({ className }: ConnectWalletProps) {
  ...
  const [isUnisatConnected, setIsUnisatConnected] = useState(false);
  const [isMagicEdenConnected, setIsMagicEdenConnected] = useState(false);
  ...
  const getXverseInscriptions = async () => {
    try {
      ...
      if (walletConnect.status === 'success') {		
        ...
        if (walletConnect.error?.code === RpcErrorCode.USER_REJECTION) {
          console.log('User rejected permissions request.');
        } else {
        console.error('Error connecting wallet:', walletConnect.error);
        }
      }
