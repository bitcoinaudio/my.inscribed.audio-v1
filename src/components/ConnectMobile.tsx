  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <span className="text-lg">{wallet.name.replace(/[-_]/g, " ").split(" ").map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(" ")}</span>
      </div>
...
export default function ConnectWallet({ className }: ConnectWalletProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className={cn("bg-white dark:bg-gray-900 border-none", "text-black dark:text-white rounded-3xl mx-auto", "fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2", "w-[480px] max-h-[560px]", "flex flex-col overflow-hidden p-0")}>
